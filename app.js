...
// ============================================================================
// PATCH GERBANG KEAMANAN MUTLAK (SECURITY GATE & AUTHENTICATION)
// ============================================================================

// 1. STATUS GEMBOK UTAMA (Default: Terkunci)
window.isAuthenticated = false;

// Simpan fungsi pindah layar yang asli dari Super Rescue
if (!window.switchLayarAsliTanpaGembok) {
    window.switchLayarAsliTanpaGembok = window.switchRoleView;
}

// 2. FUNGSI SATPAM CEGATAN (MENCEGAH TEROBOSAN DARI TOMBOL HTML)
window.switchRoleView = function(role) {
    // Jika ada yang mencoba masuk ke Owner atau Kasir TAPI gembok belum dibuka (belum login)
    if ((role === 'owner' || role === 'kasir') && window.isAuthenticated === false) {
        
        // Catat dia mau masuk ke mana
        window.targetLoginRole = role; 
    ...
    // Jika user menekan tombol "Kembali ke Halaman Customer / Keluar"
    if (role === 'customer') {
        window.isAuthenticated = false; // KUNCI KEMBALI GEMBOKNYA
        window.isKasirMode = false;
        localStorage.removeItem('isOwnerInKasir');
...
window.prosesLogin = function() {
    ...
    if (pin === window.profilOwner.pin) {
        ...
        if (errorEl) {
            errorEl.classList.remove('hidden');
            errorEl.innerText = "PIN Salah atau Tidak Terdaftar!";
        } else {
            alert("PIN Salah atau Tidak Terdaftar!");
        }
    }

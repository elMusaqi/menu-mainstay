...
window.switchRoleView = (role) => {
    ...
    if (role === 'kasir') {
        ...
        if (typeof window.requestWakeLock === 'function') window.requestWakeLock(); // Cek sebelum dipanggil
        
        const fab = document.getElementById('fab-kasir-container');
        if(fab) fab.classList.remove('hidden');
    } else if (role === 'owner') {
        if (typeof window.matikanBGM === 'function') window.matikanBGM();
        if (typeof window.releaseWakeLock === 'function') window.releaseWakeLock(); // Cek sebelum dipanggil
    } else {
...
if (typeof window.switchRoleView === 'function') {
    ...
    window.switchRoleView = (role) => {
        localStorage.setItem('mainstay_session_role', role);
        if (role === 'kasir' && typeof activeStaff !== 'undefined' && activeStaff) {
            localStorage.setItem('mainstay_session_staff', JSON.stringify(activeStaff));
...
// ==========================================
// LOGIKA FITUR POS KASIR MANUAL & WA OWNER (TAHAP 2)
// ==========================================

let posKeranjang = [];
let posTotalTagihan = 0;

// 1. Fungsi Buka WA Owner
window.hubungiOwner = () => {

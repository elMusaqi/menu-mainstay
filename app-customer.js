            ...
            return `
                <div class="bg-white rounded-3xl p-3 border shadow-sm flex flex-col justify-between relative overflow-hidden">
                    ...
                    <div class="mt-3 pt-2 border-t flex items-center justify-between gap-1">
                        ...
                        <button type="button" onclick="openOptionModal(${item.id})" class="btn-press px-2.5 py-1.5 bg-themebrand-100 text-themebrand-900 rounded-xl text-xs font-bold">+ Tambah</button>
                    </div>
                </div>
            `;
        }).join('');
        ...
        container.innerHTML = filtered.map(item => {
            ...
            return `
                <div class="bg-white p-2.5 rounded-2xl border flex items-center justify-between text-xs gap-2">
                    ...
                    <button type="button" onclick="openOptionModal(${item.id})" class="btn-press px-3 py-1.5 bg-themebrand-100 text-themebrand-900 rounded-xl font-bold">+ Tambah</button>
                </div>
            `;
        }).join('');
    }
...
// CHECKOUT ENGINE DENGAN AUTO-DRAFT WA PRESISI REAL-TIME
function submitCheckout(btnEl) {
...
function confirmPaymentAndReceipt(btnEl) {
    triggerButtonLoading(btnEl, () => {
...
function downloadReceiptPNG(btnEl) {
    triggerButtonLoading(btnEl, () => {
        showToast("Struk Digital Berhasil Diunduh!");
    });
}

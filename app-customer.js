...
function renderCustomerCategories() {
    ...
    let html = `<button type="button" onclick="filterCustomerCategory('Semua')" class="px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all ${currentCategory === 'Semua' ? 'bg-slate-900 text-amber-400 border border-slate-800 shadow' : 'bg-slate-100 text-slate-600 border border-slate-200'}">Semua Menu</button>`;
    
    store.categories.forEach(cat => {
        html += `<button type="button" onclick="filterCustomerCategory('${cat}')" class="px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all ${currentCategory === cat ? 'bg-slate-900 text-amber-400 border border-slate-800 shadow' : 'bg-slate-100 text-slate-600 border border-slate-200'}">${cat}</button>`;
    });
...
function renderCustomerMenu() {
    ...
    filtered.forEach(m => {
        ...
        if (customerLayout === 'grid') {
            grid.innerHTML += `
                <div class="bg-white rounded-2xl p-2.5 border border-slate-200 shadow-sm flex flex-col justify-between relative group hover:border-amber-400 transition-all">
                    ${isFlash ? '<span class="absolute top-2 left-2 bg-rose-600 text-white font-black text-[9px] px-2 py-0.5 rounded-md shadow z-10 animate-pulse"><i class="fa-solid fa-bolt mr-0.5"></i> FLASH SALE</span>' : ''}
                    <div class="space-y-2">
                        <div class="w-full h-28 rounded-xl overflow-hidden bg-slate-100 relative">
                            <img src="${imgUrl}" alt="${m.name}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300">
                        </div>
                        <div>
                            <span class="text-[9px] font-extrabold text-slate-400 uppercase tracking-wider">${m.category}</span>
                    ...
                    <div class="pt-2 border-t border-slate-100 flex items-center justify-between mt-2">
                        <div>
                            ${isFlash ? `<p class="text-[9px] text-slate-400 line-through">Rp ${m.price.toLocaleString('id-ID')}</p>` : ''}
                            <p class="font-black text-xs text-slate-900">Rp ${displayPrice.toLocaleString('id-ID')}</p>
                        </div>
            ...
            grid.innerHTML += `
                <div class="bg-white rounded-2xl p-3 border border-slate-200 shadow-sm flex items-center justify-between gap-3 relative hover:border-amber-400 transition-all">
                    ${isFlash ? '<span class="absolute -top-2 left-3 bg-rose-600 text-white font-black text-[9px] px-2 py-0.5 rounded-md shadow z-10"><i class="fa-solid fa-bolt mr-0.5"></i> FLASH SALE</span>' : ''}
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-1.5 mt-1">
                                ${isFlash ? `<span class="text-[10px] text-slate-400 line-through">Rp ${m.price.toLocaleString('id-ID')}</span>` : ''}
                                <span class="font-black text-xs text-slate-900">Rp ${displayPrice.toLocaleString('id-ID')}</span>
                            </div>
                        </div>
                    </div>
...
// LOGIKA JOIN MEMBER VIP & AUTO REDIRECT SALURAN WA
...
function submitJoinMember(btn) {
    ...
    // SYARAT WAJIB: Harus centang persetujuan Broadcast WA
    ...
    triggerButtonLoading(btn, () => {
        // 1. Simpan Data ke Cloud Firebase Realtime Database
        db.ref('members').child(phone).set(memberData).then(() => {
            ...
            // Auto-fill ke form checkout agar tidak perlu ketik ulang
            ...
            // 2. Direct Redirect ke Link Saluran / Group WA yang di-custom Owner
            const targetWaLink = store.theme.broadcastWaLink && store.theme.broadcastWaLink.trim() !== '' 
                ? store.theme.broadcastWaLink 
                : `https://wa.me/${(store.theme.restoPhone || "6281234567890").replace(/[^0-9]/g, '')}?text=${encodeURIComponent('Halo Admin, saya ' + name + ' (' + phone + ') baru saja mendaftar sebagai Member VIP!')}`;
            
            window.open(targetWaLink, '_blank');
        }).catch(() => {
            showToast("Gagal menyimpan member. Cek koneksi internet!", true);
        });
    });
}

// LOGIKA KUSTOMISASI VARIAN & KERANJANG
function openOptionModal(id) {
    ...
    document.getElementById('box-opt-boba').style.display = item.optBoba ? 'block' : 'none';

    selectIce('Normal Ice');
    selectSugar('100% Sugar');
    if (document.getElementById('opt-topping-boba')) document.getElementById('opt-topping-boba').checked = false;
...
function confirmAddToCartWithOptions() {
    ...
    if (hasBoba && activeOptionItem.optBoba) {
        optionsList.push("Extra Boba (+Rp 3k)");
        basePrice += 3000;
    }
    ...
    showToast(`${activeOptionItem.name} ditambahkan ke keranjang!`);
}
...
// SUBMIT CHECKOUT PESANAN KE FIREBASE CLOUD
function submitCheckout(btn) {
    ...
    triggerButtonLoading(btn, () => {
        // Push Pesanan Baru ke Cloud Firebase Realtime Database
        db.ref('orders').push(orderData).then(() => {
            ...
            nameInput.value = '';
            phoneInput.value = '';
            showToast("Pesanan berhasil terkirim ke Kasir & WA!");
        }).catch(() => {

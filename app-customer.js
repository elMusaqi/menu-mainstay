...
function renderCustomerMenu() {
    ...
    filtered.forEach(m => {
        ...
        const isClosed = !isStoreOpenCurrent;
        ...
        grid.innerHTML += `
            <div class="bg-white rounded-2xl p-2.5 border border-slate-200 shadow-sm flex flex-col justify-between group hover:border-amber-400 transition-all">
                ...
                <div class="pt-2 border-t border-slate-100 flex items-center justify-between mt-2">
                    ...
                    <button type="button" onclick="${isClosed ? "showToast('Toko sedang TUTUP!', true)" : `openOptionModal(${m.id})`}" class="btn-press px-2.5 py-1.5 ${isClosed ? 'bg-slate-300 text-slate-500 cursor-not-allowed' : 'bg-slate-900 text-amber-400'} font-extrabold text-xs rounded-xl shadow">
                        ${isClosed ? 'Tutup' : '+ Tambah'}
                    </button>
                </div>
            </div>
        `;
...
// LOGIKA MEMBER VIP (MANDATORY CONSENT + DIRECT REDIRECT LINK WA)
...
// LOGIKA KUSTOMISASI VARIAN MINUMAN & KERANJANG
function openOptionModal(id) {
    if (!isStoreOpenCurrent) {
        showToast("Toko sedang TUTUP!", true);
        return;
    }
...
// LOGIKA CHECKOUT & DRAFT WA SUPER LENGKAP
function submitCheckout(btn) {
    if (!isStoreOpenCurrent) {
        showToast("Toko sedang TUTUP! Tidak bisa mengirim order.", true);
        return;
    }
...
function executeDirectWhatsappMessage(order) {
    ...
    msg += `Follow us:\n`;
    msg += `Instagram: ${store.theme.restoIg || '@mainstay.drinkshop'}\n`;
    msg += `TikTok: ${store.theme.restoTikTok || '@mainstay.official'}\n\n`;

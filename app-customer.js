// CUSTOMER ENGINE: KATALOG, KUSTOMISASI VARIAN, KERANJANG & DIRECT WA AUTOMATION
    ...
    ['normal', 'less', 'none'].forEach(type => {
        ...
        if (btn) {
            if ((type === 'normal' && val === 'Normal Ice') || (type === 'less' && val === 'Less Ice') || (type === 'none' && val === 'No Ice')) {
                btn.className = 'opt-btn-ice py-1.5 bg-themebrand-100 text-themebrand-900 font-bold rounded-xl border border-themebrand-500 shadow-sm';
            } else {
    ...
    ['100', '50', '0'].forEach(type => {
        ...
        if (btn) {
            if ((type === '100' && val === '100% Sugar') || (type === '50' && val === '50% Sugar') || (type === '0' && val === 'No Sugar')) {
                btn.className = 'opt-btn-sugar py-1.5 bg-themebrand-100 text-themebrand-900 font-bold rounded-xl border border-themebrand-500 shadow-sm';
            } else {
...
function setOrderType(type) {
    ...
    if (type === 'now') {
        btnNow.className = 'btn-press py-2 rounded-xl font-bold border transition bg-slate-900 text-themebrand-400 ring-2 ring-themebrand-500 shadow-lg';
        ...
        btnPo.className = 'btn-press py-2 rounded-xl font-bold border transition bg-slate-900 text-themebrand-400 ring-2 ring-themebrand-500 shadow-lg';
        if (boxPo) boxPo.classList.remove('hidden');

        // Auto fill tanggal & jam preorder
...
function setPayMethod(method) {
    ...
    if (method === 'qris') {
        btnQris.className = 'btn-press py-2 rounded-xl font-bold border transition bg-slate-900 text-themebrand-400 ring-2 ring-themebrand-500 shadow-lg';
        ...
        btnCash.className = 'btn-press py-2 rounded-xl font-bold border transition bg-slate-900 text-themebrand-400 ring-2 ring-themebrand-500 shadow-lg';
    }
}
...
// FORMATTER DRAF WHATSAPP ENGINE PRESISI
function submitCheckout(btn) {
    ...
    triggerButtonLoading(btn, () => {
        ...
        // EKSEKUSI DIRECT WHATSAPP INSTANT DETIK ITU JUGA
...
function renderReceipt(order) {
    ...
    // Link Kirim Struk Ke WA Customer
...
function renderMemberStamps() {
    ...
    for (let i = 1; i <= 6; i++) {
        container.innerHTML += `
            <div class="w-6 h-6 rounded-lg bg-white/20 border border-white/40 flex items-center justify-center font-black text-[10px] text-white">
                ${i <= 2 ? '<i class="fa-solid fa-check text-white"></i>' : i}
            </div>
        `;
    }
}

// OPERASIONAL KASIR & KITCHEN PIPELINE
...
// PERBAIKAN CETAK STRUK THERMAL PUNCH HOLE 58MM
function printThermalReceipt(id) {
    ...
    if (document.getElementById('rec-schedule')) document.getElementById('rec-schedule').innerText = order.schedule;
...
// DASHBOARD OWNER MANAGEMENT & CONTROL
function setOwnerTab(tab) {
    const tabs = ['storestatus', 'branding', 'members', 'inventory', 'banners', 'staffhrm', 'ownerprofile'];
    tabs.forEach(t => {
    ...
    if (tab === 'storestatus') renderOwnerStoreStatusTab();
...
// STATUS BUKA/TUTUP TOKO & JAM OPERASIONAL
function renderOwnerStoreStatusTab() {
    const btn = document.getElementById('btn-toggle-store-manual');
    const isManualOpen = store.theme.isStoreOpenManual !== false;

    if (btn) {
        if (isManualOpen) {
            btn.className = "px-4 py-2 rounded-xl text-xs font-black shadow transition-all bg-emerald-600 text-white";
            btn.innerText = "🟢 TOKO BUKA";
        } else {
            btn.className = "px-4 py-2 rounded-xl text-xs font-black shadow transition-all bg-rose-600 text-white";
            btn.innerText = "🔴 TOKO TUTUP";
        }
    }

    if (document.getElementById('cfg-time-open')) document.getElementById('cfg-time-open').value = store.theme.openTime || "09:00";
    if (document.getElementById('cfg-time-close')) document.getElementById('cfg-time-close').value = store.theme.closeTime || "21:00";
}

function toggleStoreStatusManual() {
    store.theme.isStoreOpenManual = !(store.theme.isStoreOpenManual !== false);
    saveStore('theme');
    renderOwnerStoreStatusTab();
    checkStoreOperationalStatus();
    showToast(store.theme.isStoreOpenManual ? "Toko Di-set BUKA!" : "Toko Di-set TUTUP!", !store.theme.isStoreOpenManual);
}

function saveStoreHoursConfig(btn) {
    store.theme.openTime = document.getElementById('cfg-time-open').value;
    store.theme.closeTime = document.getElementById('cfg-time-close').value;

    saveStore('theme');
    checkStoreOperationalStatus();
    triggerButtonLoading(btn, () => showToast("Jam Operasional Berhasil Disimpan!"));
}

// BRANDING & BRAND SOSMED
function renderOwnerBrandingTab() {
    ...
    if (document.getElementById('cfg-resto-ig')) document.getElementById('cfg-resto-ig').value = store.theme.restoIg || '';
    if (document.getElementById('cfg-resto-tiktok')) document.getElementById('cfg-resto-tiktok').value = store.theme.restoTikTok || '';
...
function saveBrandingConfig(btn) {
    ...
    store.theme.restoIg = document.getElementById('cfg-resto-ig').value;
    store.theme.restoTikTok = document.getElementById('cfg-resto-tiktok').value;
    ...
    saveStore('theme');
    applyBrandingUI();
    triggerButtonLoading(btn, () => showToast("Pengaturan Branding & Sosmed Tersimpan!"));
}

// DATABASE MEMBER & EXPORT CSV
...
function exportMembersCSV() {
    if (store.members.length === 0) return showToast("Tidak ada data member untuk diekspor!", true);

    let csvContent = "data:text/csv;charset=utf-8,Nama,No WA,Waktu Bergabung\n";
    store.members.forEach(m => {
        csvContent += `"${m.name}","${m.phone}","${m.joinedAt || ''}"\n`;
    });

    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `Database_Member_${Date.now()}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    showToast("Database Member berhasil diunduh (CSV)!");
}

// MANAJEMEN INVENTORY MENU
...
function submitMenuItem(btn) {
    ...
    if (!name || price <= 0) return showToast("Mohon isi Nama Menu & Harga!", true);
...
// PROFIL OWNER & DOWNLOAD GAMBAR QRIS
function renderOwnerProfileTab() {
    ...
    if (document.getElementById('cfg-ow-qris-url')) {
        const url = store.ownerProfile.qrisUrl || 'https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=MAINSTAY-QRIS-DEMO';
        document.getElementById('cfg-ow-qris-url').value = url;
        if (document.getElementById('qris-preview-img')) document.getElementById('qris-preview-img').src = url;
    }
}
...
function saveOwnerProfile(btn) {
    ...
    const qrisUrl = document.getElementById('cfg-ow-qris-url').value;
    store.ownerProfile.qrisUrl = qrisUrl;

    if (document.getElementById('qris-preview-img')) document.getElementById('qris-preview-img').src = qrisUrl;
...
function downloadQRISImage() {
    ...
    if (!qrisImg || !qrisImg.src) return sh

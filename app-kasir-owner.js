...
function renderKasirPipeline() {
    ...
    cookingOrders.forEach(o => {
        ...
        let msgCooking = `*MAINSTAY DRINK SHOP - PESANAN SEDANG DIBUAT*\n----------------------------------------\n📋 *ID Order:* ${o.id}\nHalo Kak ${o.name}! Pembayaran Anda telah kami verifikasi LUNAS. Pesanan Anda saat ini sedang dalam proses pembuatan oleh Barista kami. Mohon ditunggu ya!`;
        ...
        listCooking.innerHTML += `
            <div class="bg-blue-50/50 p-3 rounded-2xl border border-blue-200 space-y-2 text-xs shadow-sm">
                ...
                <div class="grid grid-cols-2 gap-1.5 pt-1">
                    <a href="https://wa.me/${cleanPhone}?text=${encodeURIComponent(msgCooking)}" target="_blank" class="btn-press bg-slate-900 text-themebrand-400 font-bold text-center py-1.5 rounded-xl text-[10px] shadow-sm flex items-center justify-center gap-1">
                        <i class="fa-brands fa-whatsapp text-xs"></i> Struk & WA
                    </a>
                    <button type="button" onclick="updateOrderStatus('${o.id}', 'Completed')" class="btn-press bg-blue-600 text-white font-black py-1.5 rounded-xl text-[10px] shadow-sm">
...
function downloadSelectedQrPNG(btn) {
    showToast("Mengunduh Gambar QR Display Toko...");
}

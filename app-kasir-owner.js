// KASIR & OWNER ENGINE: PIPELINE BAR ANTREAN, EDIT STAF HRM, LOG ABSENSI, GRAFIK OMSET & QR POLOS MENTAH
...
function renderKasirPipeline() {
    ...
    // BAR ANTREAN SEDANG DIBUAT (3 TOMBOL: STRUK WA, CETAK THERMAL, SELESAI)
    cookingOrders.forEach(o => {
        let cleanPhone = o.phone.replace(/[^0-9]/g, '');
        if (cleanPhone.startsWith('0')) cleanPhone = '62' + cleanPhone.substring(1);

        let msgCooking = `*MAINSTAY DRINK SHOP - STRUK PESANAN SEDANG DIBUAT*\n----------------------------------------\n📋 *ID Order:* ${o.id}\n Waktu: ${o.timestamp}\n\n👤 Pemesan: ${o.name}\n🥤 *Item:* ${o.items.map(i => i.name).join(', ')}\n💰 *TOTAL LUNAS: Rp ${o.total.toLocaleString()}*\n----------------------------------------\nHalo Kak ${o.name}! Pesanan Anda saat ini sedang dalam proses pembuatan. Terima kasih!`;
        ...
        listCooking.innerHTML += `
            <div class="bg-blue-50/50 p-3 rounded-2xl border border-blue-200 space-y-2 text-xs shadow-sm">
                ...
                <div>
                    ...
                    <p class="text-[10px] text-slate-500 font-medium">${o.items.map(i => i.name).join(', ')}</p>
                    <p class="font-black text-blue-700 mt-0.5">Rp ${o.total.toLocaleString()}</p>
                </div>
                <div class="grid grid-cols-3 gap-1 pt-1">
                    <a href="https://wa.me/${cleanPhone}?text=${encodeURIComponent(msgCooking)}" target="_blank" class="btn-press bg-slate-900 text-themebrand-400 font-bold text-center py-1.5 rounded-xl text-[9px] shadow-sm flex items-center justify-center gap-1 truncate">
                        <i class="fa-brands fa-whatsapp"></i> Struk WA
                    </a>
                    <button type="button" onclick="printPhysicalOrder('${o.id}')" class="btn-press bg-slate-100 text-slate-800 font-bold border py-1.5 rounded-xl text-[9px] truncate">
                        <i class="fa-solid fa-print"></i> Thermal
                    </button>
                    <button type="button" onclick="updateOrderStatus('${o.id}', 'Completed')" class="btn-press bg-blue-600 text-white font-black py-1.5 rounded-xl text-[9px] shadow-sm truncate">
    ...
    completedOrders.forEach(o => {
        ...
        let msgReceipt = `*MAINSTAY DRINK SHOP - STRUK DIGITAL LUNAS*\n----------------------------------------\n📋 *ID Order:* ${o.id}\n Waktu: ${o.timestamp}\n\n👤 Pemesan: ${o.name}\n💰 *TOTAL LUNAS: Rp ${o.total.toLocaleString()}*\n----------------------------------------\nHalo Kak ${o.name}! Pesanan Anda sudah selesai dibuat dan siap diambil. Terima kasih!`;
...
function saveBrandingConfig(btn) {
    ...
    store.theme.imgGrab = document.getElementById('cfg-img-grab').value;
    store.theme.imgGoFood = document.getElementById('cfg-img-gofood').value;
    store.theme.imgIg = document.getElementById('cfg-img-ig').value;
    store.theme.imgTikTok = document.getElementById('cfg-img-tiktok').value;
...
// STAF & PENGGAJIAN (HRM WITH EDIT & PHOTO VIEW)
function renderStaffHRMTab() {
    const container = document.getElementById('hrm-staff-list');
    if (!container) return;

    container.innerHTML = store.staff.map(s => `
        <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-2 text-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div class="flex items-center gap-3 flex-1">
                <img src="${s.photoUrl || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=200&q=80'}" class="w-12 h-12 rounded-xl object-cover border border-slate-200 shrink-0">
                <div>
                    ...
                    <p class="text-[10px] text-slate-500 font-medium">WA: ${s.phone} • PIN: ${s.pin}</p>
                </div>
            </div>
            <div class="flex items-center gap-2 self-end sm:self-center shrink-0">
                <button type="button" onclick="editStaffProfile(${s.id})" class="px-3 py-1.5 bg-slate-900 text-themebrand-400 font-bold rounded-xl text-xs shadow-sm"><i class="fa-solid fa-pen-to-square mr-1"></i> Edit</button>
                <button type="button" onclick="deleteStaff(${s.id})" class="px-3 py-1.5 bg-rose-50 text-rose-600 font-bold rounded-xl text-xs border border-rose-200"><i class="fa-solid fa-trash-can"></i></button>
            </div>
        </div>
    `).join('');
}
...
function editStaffProfile(id) {
    const s = store.staff.find(item => item.id === id);
    if (!s) return;

    document.getElementById('hrm-editing-id').value = s.id;
    document.getElementById('hrm-staff-name').value = s.name;
    document.getElementById('hrm-staff-phone').value = s.phone;
    document.getElementById('hrm-staff-pin').value = s.pin;
    document.getElementById('hrm-staff-photo').value = s.photoUrl || '';
    document.getElementById('hrm-shift-in').value = s.shiftIn || "08:00";
    document.getElementById('hrm-shift-out').value = s.shiftOut || "16:00";
    document.getElementById('hrm-shift-tolerance').value = s.toleranceMinutes || 15;
    document.getElementById('hrm-salary-type').value = s.salaryType || "daily";
    document.getElementById('hrm-salary-rate').value = s.salaryRate || "";
    document.getElementById('hrm-late-fine').value = s.lateFinePerHour || "";

    document.getElementById('hrm-form-title').innerText = "Edit Profil Staf";
    document.getElementById('btn-submit-staff').innerText = "Perbarui Profil Staf";
    document.getElementById('btn-cancel-edit-staff').classList.remove('hidden');
}

function resetStaffForm() {
    document.getElementById('hrm-editing-id').value = '';
    ...
    document.getElementById('hrm-salary-rate').value = '';
    document.getElementById('hrm-late-fine').value = '';

    document.getElementById('hrm-form-title').innerText = "Tambah Staf Baru";
    document.getElementById('btn-submit-staff').innerText = "+ Simpan Profil Staf";
    document.getElementById('btn-cancel-edit-staff').classList.add('hidden');
}
...
function submitStaffProfile(btn) {
    ...
    const editingId = document.getElementById('hrm-editing-id').value;
    ...
    if (editingId) {
        const staffObj = store.staff.find(s => s.id === parseInt(editingId));
        if (staffObj) {
            staffObj.name = name;
            staffObj.phone = phone;
            staffObj.pin = pin;
            staffObj.photoUrl = document.getElementById('hrm-staff-photo').value;
            staffObj.shiftIn = document.getElementById('hrm-shift-in').value;
            staffObj.shiftOut = document.getElementById('hrm-shift-out').value;
            staffObj.toleranceMinutes = parseInt(document.getElementById('hrm-shift-tolerance').value) || 15;
            staffObj.salaryType = document.getElementById('hrm-salary-type').value;
            staffObj.salaryRate = parseInt(document.getElementById('hrm-salary-rate').value) || 0;
            staffObj.lateFinePerHour = parseInt(document.getElementById('hrm-late-fine').value) || 0;
        }
    } else {
        const newStaff = {
            id: Date.now(),
    ...
    triggerButtonLoading(btn, () => {
        resetStaffForm();
        renderStaffHRMTab();
        showToast("Data Staf Berhasil Disimpan!");
    });
}
...
function initSalesChart() {
    ...
    const dataValues = labels.map(() => Math.floor(Math.random() * 300000) + 150000);
...
// ==========================================
// QR STAND DISPLAY POSTER ENGINE (5 OPTIONS)
// ==========================================
...
function selectQrDesign(optionNum) {
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        if (btn) {
            if (i === optionNum) {
                btn.className = i === 5 ? 'p-2 rounded-xl font-bold border border-rose-500 bg-rose-100 text-rose-900 shadow-sm text-left col-span-2' : 'p-2 rounded-xl font-bold border border-themebrand-500 bg-themebrand-50 text-themebrand-900 shadow-sm text-left';
            } else {
                btn.className = i === 5 ? 'p-2 rounded-xl font-bold border border-rose-200 bg-rose-50 text-rose-600 text-left col-span-2' : 'p-2 rounded-xl font-bold border border-slate-200 bg-white text-slate-700 text-left';
            }
        }
    }

    const card = document.getElementById('qr-poster-card');
    const header = document.getElementById('qr-poster-header');
    const qrBox = document.getElementById('qr-code-box');
    const footer = document.getElementById('qr-poster-footer');

    if (!card || !header || !qrBox || !footer) return;

    qrBox.innerHTML = '';
    new QRCode(qrBox, {
        text: `https://mainstaydrink.shop/menu?store=${encodeURIComponent(store.theme.restoName)}`,
        width: optionNum === 5 ? 180 : 140,
        height: optionNum === 5 ? 180 : 140
    });

    if (optionNum === 5) {
        // Option 5: QR Murni Polos Mentah Tanpa Border/Gambar/Teks
        card.className = "bg-white p-2 rounded-none border-0 shadow-none max-w-xs mx-auto text-center";
        header.innerHTML = '';
        footer.innerHTML = '';
        qrBox.className = "m-0 p-0 bg-transparent border-0 shadow-none";
    } else {
        card.className = "bg-white p-5 rounded-2xl border-2 border-themebrand-500 shadow-md max-w-xs mx-auto text-center relative overflow-hidden";
        qrBox.className = "my-3 inline-block p-2 bg-white border border-slate-200 rounded-xl shadow-inner";
...
// REAL UNDUH POSTER QR CODE (PNG)
function downloadSelectedQrPNG(btn) {
    const qrCard = document.getElementById('qr-poster-card');
    if (!qrCard) return showToast("Elemen QR tidak ditemukan!", true);

    showToast("Memproses unduhan Poster QR...");
    html2canvas(qrCard, { scale: 2, useCORS: true }).then(canvas => {
        const image = canvas.toDataURL("image/png");
        const link = document.createElement('a');
        link.href = image;
        link.download = `QR_Toko_Mainstay_Opt${selectedQrDesignOption}.png`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        showToast("Gambar Poster QR Berhasil Diunduh!");
    }).catch(() => {
        showToast("Gagal mengunduh Poster QR!", true);
    });
}
